...
function ComingSoonScreen({
 ...
 const handleJoinWaitlist = async (e: React.FormEvent) => {
  ...
  try {
    ...
    const waitlistId = `${type}_${safeEmailId}`;
    const now = serverTimestamp();

    console.log("Firebase project:", process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID);
    console.log("Saving waitlist:", {
      waitlistId,
      subscriberId: safeEmailId,
      type,
      email: cleanEmail,
    });
    ...
    console.error("FULL WAITLIST ERROR:", error);
    console.error("ERROR CODE:", error?.code);
    console.error("ERROR MESSAGE:", error?.message);

    setMessage(error?.message || "Could not join waitlist. Please try again.");
  } finally {
    setIsJoining(false);
  }
};
